...
RC IX_IndexHandle::InsertEntry(void *pData, const RID &rid, int detail) {
	...
	int check_distr = -1; // check distribution
	int result_distr = -1; // result check

	if(result == -1){ // check to distribute 
		int leaflev = fileHdr.height - 1;
		leaflev--;

		int rootLeaf = false;
		if(level < 0){
		// parent is root
		// if parent, cant distribute
			if(fileHdr.height == 1){
				rootLeaf = true;
			}
		}
		...
		if(rootLeaf != true){ 
				IX_BTNode * posAtParent = path[level];
				int pos = posAtParent->FindKeyExact((const void* &) pData, rid);
					
				if(pos > 0){
				// has left sibling node
					check_distr = 1;
					int posAtSibling = pos - 1;
					// because of having sibling
					IX_BTNode * sibling = FetchNode(posAtParent->rids[posAtSibling]);
					// redistribute to sibling
					result_distr = node->DistributeNode(sibling);
				}
		}
	}
	if(result_distr == -1){
	// left sibling is full, cant distribute.
	// split signal
		check_distr = -1;
	}
	

    while (check_distr == -1 && result == -1) {
...
// also populates the path[] member variable with the path
// if there are dups (keys) along the path, the rightmost path will bepfFileHandle-
// chosen
