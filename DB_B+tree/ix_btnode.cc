...
RC IX_BTNode::DistributeNode(IX_BTNode * disNode) {
	if(disNode->GetKeysNum() + 1 > order){
	// empty check to overflow node
		return -1;
	}

	PageNum aPageId;
	aPageId = this->GetNodeRID().Page();

	//present node in right
	if(aPageId != disNode->GetRight()){
		return -1;
	}
	//insert to distribute
	void * k;
	this->GetKey(0,k); // get key of smallest value
	RID rid = this->GetAddr(0); // get rid of smallest value
	disNode->InsertNode(k, rid); // insert node

	//delete to distribute
	void * keytodel;
	GetKey(0, keytodel); // get value of keytodel
	RC rc = DeleteNode(keytodel, -1); // delete node
	if(rc == -1) return rc;
